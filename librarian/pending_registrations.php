			...
			if(isset($_POST['l_confirm']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						$body = "Your membership has been accepted by the library. You can now issue books using your account.";
			...
			if(isset($_POST['l_delete']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						$body = "Your membership has been rejected by the library. Please contact a librarian for further information.";
						include '../sendmail.php';
					}
				}
				if($requests > 0)
